...
from config import trainable_labels, Path
...
w1[:45] = np.linspace(.7, .3, 45)
w2[:45] = np.linspace(.3, .7, 45)
w3 = .2

focus_ds = FreesoundDataset(focus_meta, 'train', focus_cache, flip=True, mask=True)
train_ds = FreesoundDataset(train_meta, 'train', train_cache, flip=True, mask=True)
pesudo_ds = FreesoundDataset(noisy_meta, 'train', noisy_cache, flip=False, mask=False)
...
scheduler = MultiStepLR(optimizer, milestones=[45, 75, 90], gamma=0.1)
earlystopper = EarlyStopping(mode='max', min_delta=0.0001, patience=15, percentage=False)
best_score = 0
model_state_path = os.path.join(Path.MODEL_DIR, 'densenet161_noisy_gpu_{}.tar'.format(args.gpu))
