...
for fold in range(Setting.CV_FOLDS):
    ...
    best_score = 0
    model_state_path = os.path.join(Path.MODEL_DIR, 'train_focus_fold_{}_gpu_{}.tar'.format(fold, args.gpu))
...
sub.to_csv(os.path.join(Path.SUB_DIR, 'train_focus_avg_gpu_{}.csv'.format(args.gpu)), index=False)
